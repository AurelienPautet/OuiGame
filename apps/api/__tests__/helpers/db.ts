...
import { db, schema, pool } from "@ouigame/db";
...
async function createPlayer(overrides: any = {}) {
  userCounter += 1;
...
async function createSession(playerId: number, token = makeid(120)) {
...
async function createUserWithSession(overrides: any = {}) {
...
async function createSoloRound(
