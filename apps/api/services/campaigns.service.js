...
const repo = require("../repositories/campaigns.repo");
...
async function listCampaigns(name, playerId) {
  const rows = await repo.listCampaigns(name);
  return formatCampaigns(rows, playerId);
}

async function listMyCampaigns(name, playerId) {
...
async function getCampaignDetail(campaignId, playerId) {
...
async function createCampaign({ name, description, creatorId, levelIds }) {
...
async function deleteCampaign(campaignId, playerId) {
  const existing = await repo.getOwnedCampaign(campaignId, playerId);
  if (existing.length === 0) {
    throw new ServiceError(403, "Not your campaign");
  }
