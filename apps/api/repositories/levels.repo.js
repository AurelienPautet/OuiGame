...
const { eq, like, and, sql, desc, inArray, min, max } = require("drizzle-orm");
...
async function list({ name, type, maxPlayers }) {
  return db
    .select({
      id: levels.id,
      name: levels.name,
...
async function listMine({ name, playerId, maxPlayers }) {
  return db
    .select({
      id: levels.id,
      name: levels.name,
...
async function selectById(levelId) {
  return db
    .select({
      id: levels.id,
      name: levels.name,
...
async function selectContentById(levelId) {
  return db
    .select({
      id: levels.id,
      name: levels.name,
...
async function insertLevel({ name, content, creatorId, maxPlayers, type }) {
  return db
    .insert(levels)
    .values({
      name,
...
async function insertLevelImg(levelId, hexData) {
...
async function updateLevelImg(levelId, hexData) {
  return db
    .update(levelsImg)
...
async function selectLevelOwnedBy(levelId, playerId) {
  return db
    .select()
    .from(levels)
    .where(and(eq(levels.id, levelId), eq(levels.creatorId, playerId)));
...
async function getMinMaxPlayers(levelIds) {
  const res = await db
