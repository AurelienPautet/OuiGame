...
async function levelLeaderboard(levelId, limit) {
  return db
    .select({
...
async function globalLeaderboard(sortExpr, limit) {
  return db
    .select({
...
async function myStats(playerId) {
  const result = await db
    .select({
