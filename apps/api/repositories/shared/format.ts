...
async function getCreatorName(creatorId: number) {
  const res = await db
...
async function getImagesByLevelId(levelIds: number[]) {
  if (levelIds.length === 0) return new Map<number, string | null>();
  const rows = await db
...
async function getStatsByLevelId(levelIds: number[]) {
...
async function getSoloStatsByLevelId(levelIds: number[]) {
  if (levelIds.length === 0) return new Map();
  const rows = await db
    .select({
...
// A raw level row accepted by formatLevels: every row carries id/name/content/
// creatorId/maxPlayers/rating, and (where the source query selected them)
// type/status. Other repositories' list rows are assignable to this shape.
interface FormatLevelRow {
  id: number;
  name: string;
  content: unknown;
  creatorId: number;
...
async function formatLevels(rows: FormatLevelRow[]): Promise<LevelDTO[]> {
  ...
  return rows.map((row) => {
    ...
    // `rating` is the `COALESCE(AVG(stars), 0)` aggregate, typed `unknown` by
    // Drizzle's raw `sql` column. node-pg surfaces a numeric as a string and
    // the `0` fallback as a number, exactly the LevelDTO `string | number`
    // union; pass it through with a narrowing cast (the value is unchanged).
    const level_rating = row.rating as string | number;
    return {
      level_id: row.id,
      level_name: row.name,
...
export { parseId, getCreatorName, getImgFromLevelId, formatLevels };
export type { FormatLevelRow };
